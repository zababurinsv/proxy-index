...
let proxyClient = undefined
function proxy(request, response, r='') {
  if(r === 'connect') {
    init = true
  } else if(r === 'close') {
    init = false
    ...
    if(connect && init) {
      proxyClient = httpProxy.createProxyServer({});
      proxyClient.web(request, response, { target: `${target}:${portTcp}`});
      connect = false
    } else {
      if(!connect){
        proxyClient.web(request, response, { target: `${target}:${portTcp}`});
        ...
        response.write("</head>");
        response.write("<body>");
        response.write(`revers wstun server on port ${wsport} -> true`);
        response.write("</body>");
        response.write("</html>");
        response.end();
      }
  ...
  this.wsServerForControll.on('request', (function(_this){
    return function(request){
      ...
      if (uri.query.dst !== undefined){
        ...
        console.log('~~~~~~~~ uri.query.dst ~~~~~~~~~~~', request.httpRequest)
        ...
        console.log('~~~~~~~~~~~ target ~~~~~~~~~~~~~~', {
          target:`http://${src_address}:${portTcp}`
        })
        proxy(request,{
          target:`http://${src_address}:${portTcp}`,
        },'connect')
